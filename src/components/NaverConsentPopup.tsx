...
interface NaverConsentPopupProps {
  isOpen: boolean;
  onClose: () => void;
  onAgree: () => void;
}

const NAVER_CONSENT_ITEMS = [
  { id: 'name', text: '회원이름', isOptional: false },
  { id: 'email', text: '연락처 이메일 주소', isOptional: false },
  { id: 'nickname', text: '별명', isOptional: true },
  { id: 'profile_image', text: '프로필 사진', isOptional: true },
];

/**
 * @description 네이버 로그인 시 사용자 동의를 구하는 팝업 컴포넌트입니다.
 * @param {boolean} isOpen - 팝업 표시 여부.
 * @param {() => void} onClose - 팝업 닫기 또는 취소 시 호출될 함수.
 * @param {() => void} onAgree - 동의 버튼 클릭 시 호출될 함수.
 */
export const NaverConsentPopup = ({
  isOpen,
  onClose,
  onAgree,
}: NaverConsentPopupProps): JSX.Element | null => {
  if (!isOpen) {
    return null;
  }
  ...
  return (
    <dialog
      ...
      aria-labelledby="naver-consent-title"
      ...
      onKeyDown={(e: React.KeyboardEvent<HTMLDialogElement>) => {
        if (e.key === 'Escape') {
          ...
          onClose(); // 기본적으로 Enter/Space로 dialog 닫기 (취소와 동일하게)
        }
      }}
    ...
    >
      <div
        ...
        role="document"
      >
        <h2 id="naver-consent-title" className="mb-4 text-xl font-bold text-gray-800">
          네이버 로그인 동의
        </h2>
              ...
              <div>
                ...
                <span
                  className={`ml-1 text-xs ${
                    item.isOptional ? 'text-gray-500' : 'text-green-600 font-semibold'
                  }`}
                >
        ...
        <div className="flex flex-col space-y-3 sm:flex-row sm:justify-end sm:space-y-0 sm:space-x-3">
          ...
          <button
            type="button"
            onClick={onAgree}
            className="w-full rounded-md border border-transparent bg-green-500 px-4 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 sm:w-auto"
          >
            동의하고 계속하기
          </button>
        </div>
